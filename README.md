...
Step 2: Once the environment is set up, open a terminal and run: ssh -X [user]@[Guest IP Here]. 
 ...
 Step 6: Run the command 'xterm h1 h2 h3 h4 h5 h6 h7'. This will open separate terminals for all 7 hosts. 
  ...
  <destination IP> <scrPort> <app_type>. Our destination IP is always 10.0.0.2, because host 2 will send the packets to their intended host (hosts 3 through 7, depending on the app type). Our srcPort is always 1234, and the app_type argument is a number ranging from 1 through 5. 
